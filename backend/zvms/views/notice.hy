...
(defapi [:rule "/notice/list"
         ...
         :doc "获取用户能看到的全部通知"]
  list-notices []
  (import sqlalchemy [or_ any_])
  (success (select-many (. Notice query 
                            (filter (or_ 
                                     (Notice.id.in_ (. db session 
...
(defapi [:rule "/notice/send/user"
  ...
  (let [id (_save-notice (:id token-data))]
    (for [i targets]
      (insert (UserNotice
               :user-id (. (User.get/error i ErrorCode.USER-NOT-FOUND) id)
               :notice-id id)))
    (success)))
...
(defapi [:rule "/notice/mainmenu"
  ...
  (success :title main-menu-notice-title 
