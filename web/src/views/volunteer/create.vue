      ...
      <v-card-text>
        <v-form ref="form">
          ...
          <v-dialog
            ref="timeDialog"
            v-model="modalTime"
            v-model:return-value="form.time"
            persistent
            width="290px"
          >
            <template v-slot:activator="{ on, attrs }">
...
export default {
  ...
  methods: {
    async pageload() {
      ...
      for (const cls of this.classes) this.mp[cls.id] = cls.name;
    },
    createVolunteer: async function () {
      if (this.$refs.form.validate()) {
        ...
        if (
          this.form.stuMax != parseInt(this.form.stuMax) ||
          isNaN(parseInt(this.form.stuMax)) ||
          parseInt(this.form.stuMax) <= 0 ||
          this.form.inside != parseInt(this.form.inside) ||
          parseInt(this.form.inside) < 0 ||
          this.form.outside != parseInt(this.form.outside) ||
          parseInt(this.form.outside) < 0 ||
          this.form.large != parseInt(this.form.large) ||
          parseInt(this.form.large) < 0
        ) {
          toasts.error("数据不合法");
          return;
        }

        let data = await fApi.createVol(
          this.form.name,
          this.form.date,
          this.form.time,
